...
class MainViewModel : ViewModel() {
    ...
    fun postToServer(applicationContext: Context, failPercent: Int, index: Int) {
        val workManager = WorkManager.getInstance(applicationContext)

        val uuid = PostToServerWorker.submitPost(applicationContext, 3000, failPercent, "Example-$index")
